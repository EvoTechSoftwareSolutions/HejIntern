...
interface Theme {
  ...
  is_active: boolean;
  cover_image?: ThemeCoverImage | null;
}
...
// ─── Theme Card Icon ──────────────────────────────────────────────────────────
// Prefer the cover_image from the API; fall back to local icon by display_order.

const ThemeIcon = ({ theme }: { theme: Theme }) => {
  const iconClass = "w-full h-full object-contain transition-opacity duration-150 group-hover:opacity-95";

  if (theme.cover_image?.file_url) {
    return (
      <img
        src={theme.cover_image.file_url}
        alt={theme.cover_image.alt_text_en ?? theme.theme_name}
        className={iconClass}
        onError={(e) => {
          // If remote image fails, fall back to local icon
          const fallback = FALLBACK_ICONS[theme.display_order + 1];
  ...
  const localIcon = FALLBACK_ICONS[theme.display_order + 1];
  return localIcon ? (
    <img src={localIcon} alt={theme.theme_name} className={iconClass} />
  ) : null;
};
...
const Home = () => {
  ...
  const testimonials = [
    { id: 1, name: 'Ethan Wilson', country: 'Sweden', text: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.', avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=200&auto=format&fit=crop' },
    { id: 2, name: 'Ethan Wilson', country: 'Sweden', text: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.', avatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?q=80&w=200&auto=format&fit=crop' },
    { id: 3, name: 'Ethan Wilson', country: 'Sweden', text: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.', avatar: 'https://images.unsplash.com/photo-1517849845537-1d51a20414de?q=80&w=200&auto=format&fit=crop' },
  ];

  const stays = [
    { id: 1, name: 'Amanwella Resort',        location: 'Tangalle, Southern Province',     rating: 4.5, image: stay },
    { id: 2, name: 'Heritance Kandalama',      location: 'Dambulla, Central Province',      rating: 4.8, image: stay },
    { id: 3, name: 'Cape Weligama',            location: 'Weligama, Southern Province',     rating: 4.7, image: stay },
    { id: 4, name: 'Jetwing Surf',             location: 'Arugam Bay, Eastern Province',    rating: 4.6, image: stay },
    { id: 5, name: 'Wild Coast Tented Lodge',  location: 'Yala, Southern Province',         rating: 4.9, image: stay },
    { id: 6, name: 'Santani Wellness Resort',  location: 'Kandy, Central Province',         rating: 4.7, image: stay },
    { id: 7, name: 'Tri Lanka',                location: 'Koggala Lake, Southern Province', rating: 4.8, image: stay },
    { id: 8, name: 'The Fortress Resort',      location: 'Koggala, Southern Province',      rating: 4.6, image: stay },
  ];
  ...
  useEffect(() => {
    const fetchThemes = async () => {
      try {
        ...
        setThemesError(null);
        // getThemes() returns response.data.data from axios (the raw Theme array)
        const data: Theme[] = await getThemes();
        setThemes(data);
        // Pre-select the first active theme
        const first = data.find((t) => t.is_active);
        if (first) setActiveTheme(first.id);
      } catch (err: unknown) {
...
useEffect(() => {
  const fetchDestinations = async () => {
    try {
      ...
      // optional: auto-select first destination
      const first = data.find((d) => d._id);
      if (first) {
        console.log("First destination:", first);
      }
  ...
  const prevTestimonial = () => setCurrentSlide((s) => (s - 1 + testimonials.length) % testimonials.length);
  const nextTestimonial = () => setCurrentSlide((s) => (s + 1) % testimonials.length);
  ...
  const handlePointerUp = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (dx < -60) nextTestimonial();
    else if (dx > 60) prevTestimonial();
  };
  ...
  const scrollCarousel = (ref: React.MutableRefObject<HTMLDivElement | null>, direction: 'prev' | 'next') => {
    const el = ref.current;
    if (!el) return;
    const card = el.firstElementChild as HTMLElement;
    el.scrollBy({ left: (direction === 'prev' ? -1 : 1) * ((card?.offsetWidth ?? 260) + 20), behavior: 'smooth' });
  };
  ...
  return (
    <div className="w-full font-sans text-dark bg-white">
      ...
      <section className="relative w-full min-h-[100svh] lg:min-h-screen flex flex-col justify-center items-center text-center text-secondary">
        <img src={heroBanner} alt="" aria-hidden="true" className="absolute inset-0 z-0 h-full w-full object-cover object-[center_40%] sm:object-center" />
        ...
        <div className="absolute top-1/2 -translate-y-1/2 right-6 md:right-12 z-20 flex flex-col gap-3 items-center">
          {[0, 1, 2, 3, 4, 5, 6].map((index) => (
            ...
            >
              {heroSlide === index && (
                <div className="w-full h-full rounded-full bg-cover bg-center ring-2 ring-white/80" style={{ backgroundImage: `url(${heroBanner})` }} />
              )}
            </button>
          ))}
        </div>
      ...
      <section className="py-8 bg-white max-w-7xl mx-auto text-center -mt-6 sm:-mt-8 lg:mt-2 relative z-20">
        ...
        {/* Loading */}
        {themesLoading && (
          <div className="flex justify-center items-center gap-2 py-10 text-[#01888E] text-[14px]">
        ...
        {!themesLoading && themesError && (
          <div className="py-6 text-center text-[13px] text-red-500">Could not load themes: {themesError}</div>
        )}
            ...
            {themes
              ...
              .map((theme) => (
                <div
                  ...
                  className={`group flex flex-col items-center justify-center gap-1.5 rounded-[10px] p-2 flex-shrink-0
                    w-[calc((100%_-_18px)/7)] max-w-[calc((100%_-_18px)/7)] min-w-0 cursor-pointer
                    h-[96px] sm:h-[110px] transition-all duration-200
                    ${activeTheme === theme.id
                      ? 'bg-[#01888E]/15 ring-2 ring-[#01888E]/50 shadow-sm'
                      : 'bg-[#EAF5F5] hover:bg-[#01888E]/10'
                    }`}
                >
                  {/* Icon / cover image */}
                  <div className="w-[30px] h-[30px] sm:w-[36px] sm:h-[36px] md:w-[40px] md:h-[40px] flex items-center justify-center rounded-md overflow-hidden shrink-0">
                    <ThemeIcon theme={theme} />
                  </div>

                  {/* Name */}
                  <span className="text-[7px] sm:text-[8px] md:text-[9px] font-bold text-center leading-[9px] sm:leading-[10px] md:leading-[11px] text-[#003032] whitespace-pre-line break-words max-w-full px-0.5">
                    {theme.theme_name}
                  </span>
                </div>
              ))}
          ...
          <div className="flex flex-col md:flex-row items-start gap-12">
            <div className="flex-shrink-0 w-full md:w-[420px]">
              <div className="bg-white rounded-[18px] p-4 inline-block">
                <img src={aboutus} alt="About Hej Ceylon" className="w-[360px] md:w-[392px] h-auto md:h-[420px] object-cover rounded-[12px]" />
              </div>
            </div>
      ...
      <section className="py-10 relative w-full overflow-hidden px-4 md:pl-[104px] md:pr-8" style={{ backgroundImage: `url(${explorebg})`, backgroundRepeat: 'no-repeat', backgroundSize: 'cover', backgroundPosition: 'center' }}>
        <div className="max-w-[1200px] mx-auto relative z-10">
          <div className="mb-6">
            ...
            <h3 className="text-[34px] md:text-[40px] leading-[44px] text-[#003032] mb-3 font-bold" style={{ fontFamily: 'Inter' }}>
              <span className="font-light">A Land of </span><span className="font-bold text-[#01888E]">Diverse Wonders!</span>
            </h3>
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
              ...
              <div className="flex justify-end mt-3 md:mt-0 shrink-0">
                <div className="flex gap-[2px] md:mr-2">
                  <button onClick={() => scrollCarousel(toursRef, 'prev')} aria-label="Previous tours" className="w-10 h-8 bg-[#01888E] rounded-l-full rounded-r-none flex items-center justify-center text-white hover:bg-[#003032] transition-colors"><ChevronLeft size={16} /></button>
                  <button onClick={() => scrollCarousel(toursRef, 'next')} aria-label="Next tours"     className="w-10 h-8 bg-[#01888E] rounded-r-full rounded-l-none flex items-center justify-center text-white hover:bg-[#003032] transition-colors"><ChevronRight size={16} /></button>
                </div>
              </div>
            </div>
          </div>
...
>
  {destinations.map((dest) => (
    ...
    >
      ...
      <div className="relative w-full h-[180px] bg-gray-100 shrink-0">
        <img
          src={dest.cover_image?.file_url || explorebg}
        ...
        {/* Optional badge (replace rating if you have it later) */}
        <div className="absolute left-3 top-3 bg-[#0BA77A] text-white text-[11px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
          <MapPin size={12} className="text-[#FFC600]" />
          <span>{dest.region || "Sri Lanka"}</span>
        </div>
      ...
      <div className="p-4 flex flex-col justify-between flex-1 bg-white">
        ...
        >
          {dest.description?.slice(0, 120) || "Explore this beautiful destination in Sri Lanka."}
        </p>
        ...
        <div className="flex items-center justify-between mt-auto">
          ...
          <button
            ...
            className="text-gray-400 hover:text-[#01888E] transition-colors"
          >
 ...
 {/* <div ref={toursRef} className="flex gap-5 mb-6 overflow-x-auto snap-x snap-mandatory scroll-smooth [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            {tours.map((tour) => (
              <div key={tour.id} className="snap-start flex-shrink-0 w-[calc(25%-15px)] min-w-[260px] bg-white rounded-[12px] overflow-hidden transition-all duration-300 cursor-pointer flex flex-col">
                <div className="relative w-full h-[180px] bg-gray-100 shrink-0">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/10 to-transparent" />
                  <div className="absolute left-3 top-3 bg-[#0BA77A] text-white text-[11px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
                    <span className="text-[#FFC600] text-[12px]">★</span><span>{tour.rating}</span>
                  </div>
                  ...
                  <div className="absolute left-3 bottom-3 flex flex-col gap-1">
                    <div className="flex items-center gap-1.5 text-white text-[12px] font-semibold"><User size={13} className="text-white fill-white/20" /><span>{tour.title}</span></div>
                    <div className="flex items-center gap-1.5 text-white/90 text-[10px] font-medium"><Star size={11} className="text-white fill-white/20" /><span>{tour.location}</span></div>
                  </div>
                </div>
                <div className="p-4 flex flex-col justify-between flex-1 bg-white">
                  <div className="flex items-center gap-2 mb-2 text-[12px] font-bold" style={{ fontFamily: 'Inter' }}>
                    ...
                    <span className="text-gray-600 font-normal">From <span className="text-[#FF2A2A] font-bold">{tour.price}</span></span>
                  </div>
                  <p className="text-[12px] text-[#4A4A4A] leading-relaxed mb-4 tracking-wide" style={{ fontFamily: 'Inter' }}>Lorem Ipsum is simply dummy text of the printing and type setting industry.</p>
                  <div className="flex items-center justify-between mt-auto">
                    <div>
                    ...
                    <button aria-label="view tour" className="text-gray-400 hover:text-[#01888E] transition-colors"><ChevronRight size={18} /></button>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="flex justify-center mt-6">
            <button className="px-8 py-3 bg-[#01888E] text-white rounded-[12px] text-[15px] font-bold shadow-[0px_4px_12px_rgba(1,136,142,0.25)] hover:bg-[#006D6D] transition-colors" style={{ fontFamily: 'Inter' }}>Explore All Tours</button>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-[1200px] mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-5 mt-6">
            {destinationCards.map((card) => (
              <div key={card.id} className={`group ${card.wide ? 'md:col-span-2' : 'md:col-span-1'} h-[200px] sm:h-[240px] md:h-[260px] lg:h-[300px] rounded-[14px] overflow-hidden relative cursor-pointer`}>
                ...
                <div className="absolute left-5 bottom-5 right-5 z-10 translate-y-3 group-hover:translate-y-0 transition-transform duration-500 ease-out">
                  <h4 className={`font-bold text-[#003032] ${card.wide ? 'text-[22px]' : 'text-[20px]'}`} style={{ fontFamily: 'Inter' }}>{card.name}</h4>
                  <div className="max-h-0 overflow-hidden group-hover:max-h-24 transition-all duration-500 ease-in-out">
                    <p className={`text-[#003032]/75 leading-snug pt-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-150 ${card.wide ? 'text-[13px] max-w-[90%]' : 'text-[11px]'}`} style={{ fontFamily: 'Inter' }}>{card.description}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="flex justify-center mt-8">
            <button className="px-8 py-3.5 bg-[#01888E] text-white rounded-[12px] text-[15px] font-bold shadow-[0px_4px_12px_rgba(1,136,142,0.25)] hover:bg-[#006D6D] transition-colors" style={{ fontFamily: 'Inter' }}>Start Your Adventure Today</button>
          </div>
        </div>
      </section>
      ...
      <section className="py-10 relative w-full overflow-hidden px-4 md:pl-[104px] md:pr-8" style={{ backgroundImage: `url(${explorebg})`, backgroundRepeat: 'no-repeat', backgroundSize: 'cover', backgroundPosition: 'center' }}>
        <div className="max-w-[1200px] mx-auto relative z-10">
          <div className="mb-6">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
              <p className="text-[14px] leading-[22px] text-[#003032] font-normal max-w-[700px]" style={{ fontFamily: 'Inter' }}>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.</p>
              <div className="flex justify-end mt-3 md:mt-0 shrink-0">
                <div className="flex gap-[2px] md:mr-2">
                  <button onClick={() => scrollCarousel(staysRef, 'prev')} aria-label="Previous stays" className="w-10 h-8 bg-[#01888E] rounded-l-full rounded-r-none flex items-center justify-center text-white hover:bg-[#003032] transition-colors"><ChevronLeft size={16} /></button>
                  <button onClick={() => scrollCarousel(staysRef, 'next')} aria-label="Next stays"     className="w-10 h-8 bg-[#01888E] rounded-r-full rounded-l-none flex items-center justify-center text-white hover:bg-[#003032] transition-colors"><ChevronRight size={16} /></button>
                </div>
              </div>
            </div>
          </div>

          <div ref={staysRef} className="flex gap-5 mb-6 overflow-x-auto snap-x snap-mandatory scroll-smooth [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            {stays.map((item) => (
              <div key={item.id} className="snap-start flex-shrink-0 w-[calc(25%-15px)] min-w-[260px]">
                <div className="relative rounded-[12px] overflow-hidden shadow-[0px_4px_16px_rgba(0,0,0,0.08)] cursor-pointer group h-[260px] bg-white">
                  ...
                  <div className="absolute top-3 left-3 bg-[#01888E] text-white text-[11px] font-bold px-2 py-0.5 rounded-[6px] flex items-center gap-1 z-20">
                    <Star className="fill-[#FFC600] text-[#FFC600]" size={11} /><span>{item.rating}</span>
                  </div>
                  ...
                  <div className="absolute bottom-0 inset-x-0 p-4 bg-white group-hover:bg-transparent transition-all duration-500 ease-in-out flex flex-col justify-center h-[80px] z-10">
                    <h4 className="text-[15px] font-bold text-[#003032] group-hover:text-white mb-1.5 transition-colors duration-500" style={{ fontFamily: 'Inter' }}>{item.name}</h4>
                    <div className="flex items-center text-[#757575] group-hover:text-white/90 text-[12px] transition-colors duration-500" style={{ fontFamily: 'Inter' }}>
                      <MapPin size={12} className="mr-1 text-[#01888E] group-hover:text-white shrink-0 transition-colors duration-500" />{item.location}
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="flex justify-center mt-6">
            <button className="px-8 py-3 bg-[#01888E] text-white rounded-[12px] text-[15px] font-bold shadow-[0px_4px_12px_rgba(1,136,142,0.25)] hover:bg-[#006D6D] transition-colors" style={{ fontFamily: 'Inter' }}>View All</button>
          </div>
        </div>
      </section>
      ...
      <section className="relative w-full bg-white overflow-hidden">
        <div className="relative w-full bg-cover bg-center" style={{ backgroundImage: `url(${customize})`, height: '585px' }}>
          ...
          <div className="absolute bottom-0 left-0 right-0 flex justify-center px-4 translate-y-[40%] z-10">
            <div className="bg-white w-full max-w-[800px] mx-auto rounded-[20px] shadow-[0px_8px_40px_rgba(0,0,0,0.10)] px-8 py-10 text-center">
              ...
              <p className="text-[13px] md:text-[14px] text-[#757575] leading-relaxed max-w-[700px] mx-auto mb-7" style={{ fontFamily: 'Inter' }}>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.</p>
              <button className="px-12 py-3 bg-[#01888E] text-white rounded-[10px] text-[15px] font-bold shadow-[0px_4px_16px_rgba(1,136,142,0.35)] hover:bg-[#006D6D] transition-colors min-w-[180px]" style={{ fontFamily: 'Inter' }}>Customize</button>
            </div>
          </div>
        </div>
          ...
          >
            {testimonials.map((testimonial, index) => {
              ...
              const cardWidth = typeof window !== 'undefined'
                ? window.innerWidth < 640 ? 240 : window.innerWidth < 1024 ? 280 : 320
                : 320;
              return (
                <div
                  key={testimonial.id}
                  ...
                  style={{
                    transform: `translateX(${position * cardWidth}px)${isActive ? ' translateY(-8px) scale(1.05)' : ' scale(0.92)'}`,
                    transition: `transform ${SLIDE_TRANSITION_MS}ms ease-in-out, opacity ${SLIDE_TRANSITION_MS}ms ease-in-out`,
                    opacity: isActive ? 1 : 0.6,
                  }}
                >
                  <div className={`relative bg-white rounded-[24px] px-7 py-8 sm:px-8 sm:py-9 text-center overflow-hidden transition-all duration-700 flex flex-col items-center ${isActive ? 'w-[260px] sm:w-[300px] lg:w-[360px] min-h-[360px] shadow-[0px_8px_32px_rgba(1,136,142,0.18)]' : 'w-[240px] sm:w-[280px] lg:w-[320px] min-h-[320px] shadow-[0px_4px_16px_rgba(0,0,0,0.06)]'}`}>
                    ...
                    <div className={`rounded-full bg-cover bg-center ring-[4px] ring-[#01888E]/20 border-2 border-[#01888E] z-10 mt-2 ${isActive ? 'w-[80px] h-[80px] sm:w-[88px] sm:h-[88px]' : 'w-[68px] h-[68px] sm:w-[76px] sm:h-[76px]'}`} style={{ backgroundImage: `url(${testimonial.avatar})`, boxShadow: '0 4px 14px rgba(1, 136, 142, 0.3)' }} />
                    ...
                    <p className={`italic text-[#003032]/80 mb-6 px-4 ${isActive ? 'text-[12px] leading-[20px]' : 'text-[11px] leading-[18px]'}`} style={{ fontFamily: 'Inter' }}>"{testimonial.text}"</p>
                    <div className="mt-auto">
                      <h3 className={`text-[#003032] font-bold ${isActive ? 'text-[18px] mb-1' : 'text-[15px] mb-0.5'}`} style={{ fontFamily: 'Inter' }}>{testimonial.name}</h3>
                      <p className={`text-[#01888E] font-semibold ${isActive ? 'text-[14px]' : 'text-[12px]'}`} style={{ fontFamily: 'Inter' }}>{testimonial.country}</p>
                    </div>
                  </div>
                </div>
              );
          ...
          <div className="flex justify-center mt-6 gap-3">
            {testimonials.map((_, index) => (
              <button
                ...
                className={`rounded-full transition-all flex items-center justify-center ${currentSlide === index ? 'w-[18px] h-[18px] border-[2px] border-[#01888E] bg-white' : 'w-[14px] h-[14px] bg-white border border-[#01888E]/40 hover:border-[#01888E]/60'}`}
              >
