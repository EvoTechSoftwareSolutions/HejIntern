...
const TikTokIcon = ({ size = 16 }: IconProps) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
...
// Static route paths for footer link columns. Labels come from translations;
// paths stay fixed since they're not language-dependent.
const NAVIGATION_LINKS: { to: string; key: string }[] = [
  { to: "/", key: "home" },
...
const SUPPORT_LINKS: { to: string; key: string }[] = [
  { to: "/help-center", key: "helpCenter" },
  { to: "/account", key: "myAccount" },
  { to: "/favorites", key: "myFavorites" },
  { to: "/my-tours", key: "myTours" },
  { to: "/profile", key: "myProfile" },
];
...
const ContactBlock = ({ title, icon, children }: ContactBlockProps) => (
...
const Footer = () => {
  ...
  const navigationLinks: FooterLink[] = NAVIGATION_LINKS.map((link) => ({
    to: link.to,
    label: t(`navigation.links.${link.key}`),
  }));

  const supportLinks: FooterLink[] = SUPPORT_LINKS.map((link) => ({
    to: link.to,
    label: t(`support.links.${link.key}`),
  }));

  const quickLinks: FooterLink[] = QUICK_LINKS.map((link) => ({
    to: link.to,
    label: t(`quickLinks.links.${link.key}`),
  }));

  const socialLinks = [
    { href: "#", label: t("social.whatsapp"), Icon: WhatsAppIcon },
    { href: "#", label: t("social.facebook"), Icon: FacebookIcon },
    { href: "#", label: t("social.instagram"), Icon: InstagramIcon },
    { href: "#", label: t("social.tiktok"), Icon: TikTokIcon },
  ];
        ...
        <div className="grid grid-cols-6 sm:grid-cols-6 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="col-span-6 lg:col-span-4 flex flex-col">
            <div className="flex items-center gap-3 mb-4">
              <img src={logo} alt={t("brand.logoAlt")} className="h-[38px] w-auto" />
            ...
            <div className="flex items-center w-full max-w-[320px] bg-transparent rounded-full border-[1.5px] border-white/80 overflow-hidden mb-6">
              ...
              <button
                type="button"
                aria-label={t("newsletter.subscribeAriaLabel")}
          ...
          <div className="col-span-2 lg:col-span-2">
            <FooterLinkColumn title={t("navigation.title")} links={navigationLinks} />
          </div>

          {/* Support */}
          <div className="col-span-2 lg:col-span-2">
            <FooterLinkColumn title={t("support.title")} links={supportLinks} />
          </div>

          {/* Quick Links */}
          <div className="col-span-2 lg:col-span-2">
            <FooterLinkColumn title={t("quickLinks.title")} links={quickLinks} />
          </div>
      ...
      <div className="bg-white text-[#4B5563] relative z-20 py-3 min-h-[44px] flex items-center">
        <div className="max-w-[1200px] mx-auto flex flex-col sm:flex-row justify-between items-center w-full px-4 sm:px-6 lg:px-8 gap-2 text-[11px] sm:text-[12px]">
          <p className="text-center sm:text-left">
            © <span className="font-bold">{t("copyright.text")}</span>
          </p>
          <p className="text-center sm:text-right">
            {t("copyright.designByPrefix")}{" "}
            <span className="font-bold text-[#1F2937]">{t("copyright.designByCompany")}</span>
          </p>
        </div>
      </div>
    </footer>
